...
# Create an HTML frame to display the movie details
html_frame = HtmlFrame(window, width=800, height=500)
html_frame.pack(side="right", fill="both", expand=True)

# Create a frame to hold the buttons
frame = ttk.Frame(html_frame, padding=10)
...
def open_stream(url):
    ...
    if stream_button:
        ...
        html_frame.set_content(f'<html><body><a href="{stream_url}">Click here to play the movie</a></body></html>')
