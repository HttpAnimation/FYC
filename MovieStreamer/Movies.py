...
with open('Movies.html', 'r') as file:
    ...
    folder_name = os.path.join(home_dir, 'MoviesStreamer')
    os.makedirs(folder_name, exist_ok=True)

    print('Copying files...')
    copied_files = []

    # Copy the Movies.html file to the MoviesStreamer folder
    shutil.copy2('Movies.html', folder_name)
    copied_files.append('Movies.html')
    ...
    for button in buttons:
        ...
        if os.path.exists(url):
            ...
            # Skip the file and remove the button from Movies.html
    ...
    # Rename the copied Movies.html file to index.html
    old_file_path = os.path.join(folder_name, 'Movies.html')
    ...
    for copied_file in copied_files:
        if copied_file != 'Movies.html':
            file_path = os.path.join(folder_name, copied_file)
            if os.path.exists(file_path):
                with open(file_path, 'r') as file:
                    contents = file.read()
                    updated_contents = contents.replace('3D-Movies', 'Movies')
